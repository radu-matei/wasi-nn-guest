...
export class Graph {
  ...
  static load(
    ...
    let resultCode = wasi_ephemeral_nn.load(
      getArrayPtr(graphBuilder),
      builder.length,
      encoding,
      target,
      graphPointer
  ...
  initExecutionContext(): ExecutionContext {
    ...
    let resultCode = wasi_ephemeral_nn.init_execution_context(
      load<u32>(this.pointer),
      executionContextPointer
    );
    if (resultCode != 0) {
      throw new WasiNnError(
        "Unable to initialize an execution context",
        resultCode
      );
    }
...
export class ExecutionContext {
  ...
  setInput(index: u32, tensor: Tensor): void {
    let resultCode = wasi_ephemeral_nn.set_input(
      load<u32>(this.pointer),
      index,
      tensor.asPointer()
    );
  ...
  getOutput(index: u32, outputBuffer: Array<u8>): Tensor {
    ...
    let resultCode = wasi_ephemeral_nn.get_output(
      load<u32>(this.pointer),
      index,
...
export class Tensor {
  ...
  asPointer(): u32 {
    let struct: u32[] = [
      getArrayPtr(this.dimensions),
      this.dimensions.length,
      this.type,
      getArrayPtr(this.data),
      this.data.length,
    ];
